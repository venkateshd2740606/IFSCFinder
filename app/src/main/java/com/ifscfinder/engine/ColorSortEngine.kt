...
object IFSCFinderEngine {

    fun createInitialGame(level: IFSCFinderLevel): IFSCFinderGame = IFSCFinderGame(
        level = level,
    ...
    fun pourCount(game: IFSCFinderGame, from: Int, to: Int): Int {
    ...
    fun canPour(game: IFSCFinderGame, from: Int, to: Int): Boolean = pourCount(game, from, to) > 0

    fun pour(game: IFSCFinderGame, from: Int, to: Int): IFSCFinderGame {
    ...
    fun onTubeSelected(game: IFSCFinderGame, tubeId: Int): IFSCFinderGame {
    ...
    fun isWon(tubes: List<List<Int>>, level: IFSCFinderLevel): Boolean =
        tubes.all { tube -> tube.isEmpty() || tube.distinct().size == 1 }

    fun isWon(game: IFSCFinderGame): Boolean = isWon(game.tubes, game.level)

    fun getHintMove(game: IFSCFinderGame): Pair<Int, Int>? {
    ...
    fun solve(game: IFSCFinderGame): List<Pair<Int, Int>>? {
        if (isWon(game)) return emptyList()

        data class Node(val state: IFSCFinderGame, val path: List<Pair<Int, Int>>)
    ...
    fun optimalMoveCount(game: IFSCFinderGame): Int =
