...
import com.ifscfinder.domain.model.PuzzleArchetype
import com.ifscfinder.domain.model.PuzzleProfile
import com.ifscfinder.domain.model.PuzzleProfileMetrics
import com.ifscfinder.domain.model.IFSCFinderGame
import com.ifscfinder.domain.model.SkillCategory
...
object PuzzleProfileEngine {
    ...
    fun updateMetrics(current: PuzzleProfileMetrics, game: IFSCFinderGame): PuzzleProfileMetrics {
        if (game.status != GameStatus.COMPLETED || game.level.isTutorial) return current

        val moveCount = max(game.moves, 1)
        val optimalMoves = IFSCFinderEngine.optimalMoveCount(game)
