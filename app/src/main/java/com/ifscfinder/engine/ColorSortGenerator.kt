...
import com.ifscfinder.domain.model.GenerationProfile
import kotlin.random.Random

object IFSCFinderGenerator {
    ...
    fun generate(
        ...
        repeat(64) {
            ...
            if (IFSCFinderEngine.validateLevel(level)) return level
            attemptSeed++
        }
    ...
    fun generateForChallenge(
        ...
        levelNumber: Int,
        difficulty: Difficulty
    ): IFSCFinderLevel = generate(seed, levelNumber, difficulty)
    ...
    private fun buildLevel(
        ...
        val capacity = IFSCFinderLevel.DEFAULT_CAPACITY
    ...
    private fun scrambleFromSolved(
        ...
        while (made < moves && attempts < moves * 20) {
            ...
            if (from == to) continue
            val game = IFSCFinderEngine.createInitialGame(
                IFSCFinderLevel(
                    seed = 0,
                    levelNumber = 1,
            ...
            if (!IFSCFinderEngine.canPour(game, from, to)) continue
            val updated = IFSCFinderEngine.pour(game, from, to)
