package com.ifscfinder.data.local.database.dao
...
@Dao
interface IfscFavoriteDao {
    @Query("SELECT * FROM ifsc_favorites ORDER BY savedAt DESC")
    fun observeAll(): Flow<List<IfscFavoriteEntity>>

    @Query("SELECT * FROM ifsc_favorites WHERE ifsc = :ifsc LIMIT 1")
    suspend fun getByIfsc(ifsc: String): IfscFavoriteEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(favorite: IfscFavoriteEntity)

    @Query("DELETE FROM ifsc_favorites WHERE ifsc = :ifsc")
    suspend fun delete(ifsc: String)
}
